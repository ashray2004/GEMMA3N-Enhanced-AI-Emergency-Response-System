...
const SW_CONFIG = {
  VERSION: "v2.4.5-fix", // Updated version to trigger cache refresh
...
self.addEventListener("fetch", (event) => {
  ...
  // Never cache ai-worker.js - always fetch fresh
  if (url.pathname.includes('ai-worker.js')) {
...
const cacheManager = {
  async cacheInitialAssets() {
    ...
    // Cache files individually, skipping missing ones
    await Promise.allSettled(PRECACHE_URLS.map(url => cache.add(url).catch(e => console.warn(`Skip ${url}`))));
    const cached = PRECACHE_URLS.length;
    console.log(`Cached ${cached} essential files.`);
